...
function do_shortcodes_in_html_tags( $content, $ignore_html ) {
	...
	foreach ( $textarr as &$element ) {
		...
		$noopen = false === strpos( (string)$element, '[' );
		$noclose = false === strpos( (string)$element, ']' );
		if ( $noopen || $noclose ) {
		...
		foreach ( $attributes as &$attr ) {
			$open = strpos( (string)$attr, '[' );
			$close = strpos( (string)$attr, ']' );
			if ( false === $open || false === $close ) {
				continue; // Go to next attribute.  Square braces will be escaped at end of loop.
			}
			$double = strpos( (string)$attr, '"' );
			$single = strpos( (string)$attr, "'" );
